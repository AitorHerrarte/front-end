...
export default function QuantumPage() {
  ...
  const addNumber = async () => {
    if (newNumberData.number.trim() !== "") {
      try {
        await axios.post("https://galiyaitormoneyback-a4x7.onrender.com/quantum/addNumber/", {
    ...
    if (newNumberData.numberToCalculate.trim() !== "") {
      try {
        const response = await axios.get(
          `https://galiyaitormoneyback-a4x7.onrender.com/quantum/getAllNextNumbers/${newNumberData.numberToCalculate}`
        );
